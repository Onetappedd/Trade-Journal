...
export type TradesByDay = Record<
  string,
  Array<{
    id: string;
    symbol: string;
...
export async function getRealizedCalendarData(
  userId: string,
  startDateISO: string,
  endDateISO: string,
) {
  ...
  const { data, error } = await supabase
    .from('trades')
    .select(
      'id,user_id,symbol,side,quantity,entry_price,exit_price,entry_date,exit_date,status,pnl,asset_type',
    )
    .eq('user_id', userId)
    .or(
  ...
  for (const t of (data || []) as RealizedTrade[]) {
    ...
    if (t.status !== 'closed' || !t.exit_date || t.exit_price == null || t.entry_price == null)
      continue;
