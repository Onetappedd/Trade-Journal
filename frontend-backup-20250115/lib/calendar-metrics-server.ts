...
export async function getUserTradesGroupedByDay(
  ...
  console.log(
    `[Calendar Server] Fetching trades for user ${userId}:`,
    trades?.length || 0,
    'trades found',
  );
  ...
  console.log(
    `[Calendar Server] - Open positions: ${positions.filter((p) => p.openQuantity > 0).length}`,
  );
  ...
  for (const trade of closedTrades) {
    ...
    console.log(
      `[Calendar Server] ${dateKey}: ${trade.symbol} ${trade.side} - P&L: $${trade.pnl.toFixed(2)}`,
    );
  }
