...
export function PerformanceComparisonNew({
  ...
  const benchmarkData = useMemo(() => {
    ...
    const annualReturn =
      selectedBenchmark === 'SPY' ? 0.1 : selectedBenchmark === 'QQQ' ? 0.15 : 0.08;
                ...
                <div className="grid gap-4 md:grid-cols-4">
                  <div>
                    ...
                    <p
                      className={cn(
                        'text-lg font-bold',
                        periodBreakdown.reduce((sum, p) => sum + p.return, 0) /
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold">
                      {(
                        periodBreakdown.reduce((sum, p) => sum + p.winRate, 0) /
                          periodBreakdown.filter((p) => p.trades > 0).length || 0
                      ).toFixed(1)}
                      %
                    </p>
                  </div>
                    ...
                    <Bar dataKey="return" name="Return (%)">
                      {periodBreakdown.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={
                            entry.return > 0 ? '#10b981' : entry.return < 0 ? '#ef4444' : '#9ca3af'
                          }
                        />
                      ))}
                    </Bar>
                      ...
                      {periodBreakdown
                        ...
                        .map((period, index) => (
                          <tr key={index} className="border-b hover:bg-muted/50">
                            ...
                            >
                              {period.return > 0 ? '+' : period.return < 0 ? '-' : ''}
                              {Math.abs(period.return).toFixed(2)}%
                            </td>
                            <td
                              className={cn(
                                'text-right p-2',
                            ...
                            >
                              {period.pnl > 0 ? '+' : period.pnl < 0 ? '-' : ''}$
                              {Math.abs(period.pnl).toFixed(2)}
                            </td>
                            ...
                            >
                              {period.avgTrade > 0 ? '+' : period.avgTrade < 0 ? '-' : ''}$
                              {Math.abs(period.avgTrade).toFixed(2)}
                            </td>
                          </tr>
                        ))}
                    </tbody>
