...
function makeHeaders(...cols: string[]) {
  return cols;
}
...
describe('parsing adapters - detection + parsing', () => {
  it('ibkr: detects and parses options with OCC string, quantity x100 and contracts note', () => {
    ...
    const { fills, errors } = adapter.parse({
      rows,
      headerMap: det!.headerMap,
      userTimezone: 'America/New_York',
      assetClass: det!.assetClass,
    });
  ...
  it('webull: detection works and parses fees sum', () => {
    ...
    const rows = [
      {
        'Time Executed': '2024-02-01 10:00:00',
        Symbol: 'TSLA',
        Side: 'SELL',
        Quantity: '3',
        'Avg Price': '210.10',
  ...
  it('robinhood: detects missing time header (using date only) and parses', () => {
    ...
    const rows = [
      { Date: '2024-03-10', Symbol: 'AAPL', Side: 'Buy', Quantity: '1', Price: '180', Fees: '0' },
    ];
    const det = detectAdapter(headers, rows)!;
    expect(det.brokerId).toBe('robinhood');
    const { fills } = getAdapters()
      .find((a) => a.id === 'robinhood')!
      .parse({
        rows,
        headerMap: det.headerMap,
        userTimezone: 'America/New_York',
  ...
  it('etrade: parses action and sums fees', () => {
    ...
    const rows = [
      {
        'Transaction Date': '2024-05-10 13:45:12',
        Symbol: 'NFLX',
        Action: 'Buy',
        Quantity: '2',
        Price: '500',
  ...
  it('tradestation: detects and parses', () => {
    ...
    const rows = [
      {
        'Execution Date': '2024-07-15 09:35:00',
        Symbol: 'QQQ',
        Side: 'Sell',
        Quantity: '10',
        Price: '400',
  ...
  it('coinbase: detects and notes non-USD fee currency', () => {
    ...
    const { fills } = getAdapters()
      .find((a) => a.id === det.brokerId)!
      .parse({ rows, headerMap: det.headerMap, userTimezone: 'UTC', assetClass: det.assetClass });
  ...
  it('futures: basic canonicalization and DST-safe execTime ISO', () => {
    ...
    const rows = [
      { 'Date/Time': '2024-11-03 01:30:00', Symbol: 'NQZ4', Quantity: '1', 'T. Price': '16000' },
    ];
    const det = detectAdapter(headers, rows)!;
