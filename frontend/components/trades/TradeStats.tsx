      ...
      <CardContent>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 text-sm">
          ...
          <div>
            ...
            <p className={`font-bold text-lg tabular-nums ${stats.winRate >= 50 ? 'text-green-600' : 'text-red-600'}`}>
              {stats.winRate.toFixed(1)}%
            </p>
          </div>
          <div>
            <p className="text-muted-foreground">Total P&L</p>
            <p className={`font-bold text-lg tabular-nums ${stats.totalPnL >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              {formatCurrency(stats.totalPnL)}
            </p>
          </div>
          <div>
            <p className="text-muted-foreground">Avg P&L</p>
            <p className={`font-bold text-lg tabular-nums ${closedTrades.length > 0 && stats.totalPnL / closedTrades.length >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              {closedTrades.length > 0 ? formatCurrency(stats.totalPnL / closedTrades.length) : "$0.00"}
            </p>
          </div>
          <div>
        ...
        {positions.some(p => p.openQuantity > 0) && (
          <div className="mt-6 pt-6 border-t">
            ...
            <div className="space-y-2">
              {positions.filter(p => p.openQuantity > 0).map((position, idx) => (
