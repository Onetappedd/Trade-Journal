...
export function IntegrationSettings() {
  ...
  const toggleBrokerConnection = (brokerId: string) => {
    setBrokerStates((prev) =>
      prev.map((broker) =>
        broker.id === brokerId
          ? { ...broker, connected: !broker.connected, autoSync: false }
          : broker,
      ),
    );
  };
  ...
  const toggleAutoSync = (brokerId: string) => {
    setBrokerStates((prev) =>
      prev.map((broker) =>
        broker.id === brokerId ? { ...broker, autoSync: !broker.autoSync } : broker,
      ),
    );
  };
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Connect your brokerage accounts to automatically import trades
          </CardDescription>
        </CardHeader>
