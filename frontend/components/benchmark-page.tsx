    ...
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              +{portfolioPerformance.ytdReturn}%
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {portfolioPerformance.sharpeRatio}
            </div>
          ...
          <Table>
            ...
            <TableBody>
              <TableRow className="bg-muted/50">
                ...
                <TableCell className="text-green-600 font-medium">
                  +{portfolioPerformance.monthReturn}%
                </TableCell>
                <TableCell className="text-green-600 font-medium">
                  +{portfolioPerformance.weekReturn}%
                </TableCell>
                <TableCell>-</TableCell>
                <TableCell>-</TableCell>
              </TableRow>
              {benchmarkData.map((benchmark) => {
                ...
                const monthOutperformance =
                  portfolioPerformance.monthReturn - benchmark.monthReturn;
                ...
                return (
                  <TableRow key={benchmark.symbol}>
                    ...
                    <TableCell
                      className={benchmark.ytdReturn >= 0 ? 'text-green-600' : 'text-red-600'}
                    >
                      {benchmark.ytdReturn >= 0 ? '+' : ''}
                      {benchmark.ytdReturn}%
                    </TableCell>
                    <TableCell
                      className={benchmark.monthReturn >= 0 ? 'text-green-600' : 'text-red-600'}
                    >
                      {benchmark.monthReturn >= 0 ? '+' : ''}
                      {benchmark.monthReturn}%
                    </TableCell>
                    <TableCell
                      className={benchmark.weekReturn >= 0 ? 'text-green-600' : 'text-red-600'}
                    >
          ...
          <div className="h-[400px] flex items-center justify-center border-2 border-dashed border-gray-300 rounded-lg">
            <div className="text-center">
              ...
              <p className="text-xs text-gray-400">
                Portfolio vs S&P 500, NASDAQ, and other benchmarks
              </p>
            </div>
          </div>
        </CardContent>
