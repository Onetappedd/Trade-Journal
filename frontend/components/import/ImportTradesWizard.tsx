...
function validateRows(rows: any[], mapping: Record<string,string>, assetClass: string): { errors: Array<{row:number; message:string}>; warnings: string[]; duplicates: number; invalidRows: Set<number> } {
  ...
  rows.forEach((r, idx) => {
    ...
    const fee = fCol && r[fCol] !== undefined ? Number(String(r[fCol]).replace(/,/g, "")) : undefined
    if (!Number.isFinite(q)) { errors.push({ row: rowNum, message: "Quantity is not a number" }); invalidRows.add(idx) }
    if (!Number.isFinite(p)) { errors.push({ row: rowNum, message: "Price is not a number" }); invalidRows.add(idx) }
    if (fee !== undefined && !Number.isFinite(fee)) { warnings.push(`Row ${rowNum}: Fees not numeric, ignored`) }
    // Timestamp
    ...
    if (!t || isNaN(t.getTime())) { errors.push({ row: rowNum, message: "Invalid timestamp" }); invalidRows.add(idx) }
    // Aggregates
...
export function ImportTradesWizard() {
  const [step, setStep] = useState<"chooseBroker"|"chooseAssetType"|"upload"|"detecting"|"preview"|"importing"|"done"|"error">("chooseBroker")
  const [brokerId, setBrokerId] = useState<string | undefined>(undefined)
  const [assetClass, setAssetClass] = useState<"stocks"|"options"|"futures"|"crypto"|undefined>(undefined)
  const [file, setFile] = useState<File | undefined>(undefined)
  const [fileInfo, setFileInfo] = useState<{ name: string; size: number; type: string } | undefined>(undefined)
  ...
  const { errors, warnings, duplicates, invalidRows } = useMemo(() => validateRows(rows, headerMap, assetClass || "stocks"), [rows, headerMap, assetClass])
  const previewRowsRaw = useMemo(() => buildPreview(rows, headerMap, 50), [rows, headerMap])
  const previewRows = useMemo(() => skipInvalid ? previewRowsRaw.filter((_, idx) => !invalidRows.has(idx)) : previewRowsRaw, [previewRowsRaw, skipInvalid, invalidRows])
  const totals = useMemo(() => computeTotals(previewRows), [previewRows])

  // Errors CSV
  useEffect(() => {
    if (!errors.length) { if (errorsCsvUrl) URL.revokeObjectURL(errorsCsvUrl); setErrorsCsvUrl(undefined); return }
    const header = ["row", "message"]
    const lines = [header.join(","), ...errors.map(e => `${e.row},"${e.message.replace(/"/g,'""')}"`)]
      ...
      <Dialog open={mappingOpen} onOpenChange={setMappingOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>Adjust how your file headers map to normalized fields.</DialogDescription>
          </DialogHeader>
              ...
              <div className="space-y-1">
                {NORMALIZED_FIELDS.map((f) => (
                  <div key={f} className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-xs text-muted-foreground">{Object.keys(localMap).find(k => localMap[k] === f) || "(unassigned)"}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {BROKERS.map(broker => (
              <Button key={broker.id} variant={brokerId === broker.id ? "default" : "outline"} className="flex flex-col items-center p-4" onClick={() => setBrokerId(broker.id)}>
                <span className="font-semibold mt-2">{broker.name}</span>
              </Button>
            ))}
          </div>
          <div className="mt-4 flex justify-between">
            <Button variant="outline" disabled={!canNextFromBroker} onClick={() => setStep("chooseAssetType")}>Next</Button>
            <Button variant="ghost" onClick={() => setHelpOpen(true)} disabled={!brokerId}>Where to find your export?</Button>
          </div>
          <Dialog open={helpOpen} onOpenChange={setHelpOpen}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>How to export from {BROKERS.find(b => b.id === brokerId)?.name || "your broker"}</DialogTitle>
              </DialogHeader>
              {(() => {
                // Dynamic help content from config
                const { BROKER_EXPORT_GUIDES } = require("@/config/broker-export-guides")
                const guide = (brokerId && BROKER_EXPORT_GUIDES[brokerId]) || BROKER_EXPORT_GUIDES.generic
                return (
                  <div className="space-y-3">
                    ...
                    {guide.notes && guide.notes.length > 0 && (
                      <div className="text-xs text-muted-foreground space-y-1">
                        {guide.notes.map((n: string, i: number) => (<div key={i}>• {n}</div>))}
                      </div>
                    )}
                  </div>
                )
        ...
        <CardContent>
          <div className="flex flex-wrap gap-4">
            {ASSET_CLASSES.map(asset => (
              <Button key={asset.id} variant={assetClass === asset.id ? "default" : "outline"} onClick={() => setAssetClass(asset.id as any)}>{asset.label}</Button>
            ))}
          </div>
          <div className="mt-4 flex justify-between">
            <Button variant="outline" onClick={() => setStep("chooseBroker")}>Back</Button>
            <Button variant="outline" disabled={!canNextFromAsset} onClick={() => setStep("upload")}>Next</Button>
          </div>
        </CardContent>
      </Card>
    )
        ...
        <CardContent>
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              <Button variant="outline" onClick={() => setStep("chooseAssetType")}>Back</Button>
              <Button variant="default" disabled={!canNextFromUpload} onClick={runDetect}>Analyze</Button>
            </div>
          </div>
        </CardContent>
      </Card>
    ...
    return (
      <Card>
        ...
        <CardContent>
          ...
          <div className="mt-4"><Button variant="outline" onClick={() => setStep("upload")}>Cancel</Button></div>
        </CardContent>
      </Card>
    )
  }
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setMappingOpen(true)}>Re-map columns</Button>
              {errorsCsvUrl && <a href={errorsCsvUrl} download="import-errors.csv"><Button variant="outline">Download Errors CSV</Button></a>}
            </div>
          </div>
          <div className="text-sm text-muted-foreground mt-2">
            Rows: {rows.length} • Errors: {errors.length} • Warnings: {warnings.length} • Duplicates: {duplicates}
          </div>
          <div className="flex items-center gap-2 mt-2">
            <input id="skipInvalid" type="checkbox" checked={skipInvalid} onChange={e => setSkipInvalid(e.target.checked)} />
            <Label htmlFor="skipInvalid">Skip invalid rows</Label>
            {skipInvalid && <span className="text-xs text-muted-foreground">(skipping {invalidRows.size})</span>}
          </div>
        </CardHeader>
        <CardContent>
          {/* Column badges */}
                  ...
                  {headers.map(h => (
                    <th key={h} className="px-2 py-1 border-b text-left">
                      <div className="flex flex-col">
                        <span>{h}</span>
                        <span className="text-xs text-muted-foreground">{headerMap[h] || "(unassigned)"}</span>
                      </div>
                    </th>
                  ))}
                </tr>
                ...
                {rows.slice(0,50).map((row, idx) => {
                  ...
                  return (
                    <tr key={idx} className="border-t">
                      {headers.map(h => (
                        <td key={h} className="px-2 py-1 align-top">{String(row[h] ?? "")}</td>
                      ))}
                    </tr>
                  )
                })}
          ...
          <div className="mt-4 text-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
              ...
              <div>
                ...
                <div>{totals.earliest ? new Date(totals.earliest).toLocaleString() : "—"} → {totals.latest ? new Date(totals.latest).toLocaleString() : "—"}</div>
              </div>
            </div>
          </div>
          <div className="mt-4 flex justify-between">
            <Button variant="outline" onClick={() => setStep("upload")}>Back</Button>
            <Button variant="default" disabled={allInvalid} onClick={() => setStep("importing")}>Continue</Button>
          </div>
        </CardContent>
        <MappingDrawer />
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="mt-4 flex justify-between">
            ...
            <Button variant="default" onClick={() => setStep("done")}>Finish</Button>
          </div>
        </CardContent>
      </Card>
    )
  ...
  if (step === "done") {
    return (
      <Card>
        ...
        <CardContent>
          <div className="mb-4">Summary will appear here (rows imported, duplicates skipped, errors file, impacted accounts, earliest/latest fill).</div>
          <Button variant="default" onClick={() => { setStep("chooseBroker"); setBrokerId(undefined); setAssetClass(undefined); setFile(undefined); setDetect(undefined); setHeaderMap({}); setHeaders([]); setRows([]) }}>Start Over</Button>
        </CardContent>
      </Card>
    )
  }
  ...
  if (step === "error") {
    return (
      <Card>
        ...
        <CardContent>
          <div className="text-red-600 mb-4">An error occurred. Please review your file and try again.</div>
          <Button variant="default" onClick={() => setStep("chooseBroker")}>Start Over</Button>
        </CardContent>
      </Card>
    )
  }
