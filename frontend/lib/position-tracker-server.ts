// Server-side position tracking logic (no client-side dependencies)
...
export function calculatePositions(trades: Trade[]): {
  ...
  console.log(`[Position Tracker] Processing ${sortedTrades.length} trades`)
  ...
  for (const trade of sortedTrades) {
    ...
    if (trade.side === 'buy' || trade.side === 'Buy' || trade.side === 'BUY') {
      ...
      console.log(`[Position Tracker] BUY ${trade.symbol}: ${trade.quantity} @ ${trade.entry_price}, Open: ${position.openQuantity}`)
      ...
      if (closedQuantity > 0) {
        ...
        console.log(`[Position Tracker] SELL ${trade.symbol}: ${closedQuantity} @ ${trade.entry_price}, Avg Entry: ${position.avgEntryPrice}, P&L: ${pnl}`)
        ...
        console.log(`[Position Tracker] SELL ${trade.symbol}: No open position to close`)
      }
    }
  }
  ...
  console.log(`[Position Tracker] Closed trades: ${closedTrades.length}, Total P&L: ${totalPnL}`)
  console.log(`[Position Tracker] Winning: ${winningTrades.length}, Losing: ${losingTrades.length}`)
  console.log(`[Position Tracker] Open positions: ${openPositions}`)
