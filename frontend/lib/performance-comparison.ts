  ...
  try {
    ...
    for (let i = 0; i < timestamps.length; i++) {
      if (closes[i] !== null) {
        ...
        const percentChange = previousClose ? ((closes[i] - previousClose) / previousClose) * 100 : 0
...
export async function calculateUserPerformance(
  ...
  for (const trade of trades) {
    ...
    if (trade.exit_price && trade.exit_date) {
      ...
      const pnl = (trade.exit_price - trade.entry_price) * trade.quantity * multiplier * (trade.side === 'buy' ? 1 : -1)
...
export async function compareToBenchmark(
  ...
  // Calculate returns
  const userReturn = userPerf.length > 0 
    ? ((userPerf[userPerf.length - 1].value - userPerf[0].value) / userPerf[0].value) * 100
    : 0
    
  const benchmarkReturn = benchmarkPerf.length > 0
    ? ((benchmarkPerf[benchmarkPerf.length - 1].value - benchmarkPerf[0].value) / benchmarkPerf[0].value) * 100
    : 0
