...
class PolygonService {
  ...
  async getMarketMovers(direction: 'gainers' | 'losers' = 'gainers'): Promise<PolygonMarketMover[]> {
    try {
      const data = await this.request<{ results: PolygonMarketMover[] }>(`/v2/snapshot/locale/us/markets/stocks/${direction}`)
      return data.results || []
    } catch (error) {
  ...
  async getTickerSnapshot(ticker: string): Promise<PolygonSnapshot | null> {
    try {
      const data = await this.request<{ results: PolygonSnapshot[] }>(`/v2/snapshot/locale/us/markets/stocks/tickers/${ticker.toUpperCase()}`)
  ...
  async getBatchSnapshots(tickers: string[]): Promise<PolygonSnapshot[]> {
    try {
      ...
      const data = await this.request<{ results: PolygonSnapshot[] }>('/v2/snapshot/locale/us/markets/stocks/tickers', {
        'tickers': tickerList
      })
      return data.results || []
    } catch (error) {
  ...
  async getTickerDetails(ticker: string): Promise<PolygonTickerDetails | null> {
    try {
      const data = await this.request<{ results: PolygonTickerDetails }>(`/v3/reference/tickers/${ticker.toUpperCase()}`)
  ...
  async getAllMarketMovers(): Promise<{
    ...
    try {
      ...
      const popularTickers = ['AAPL', 'TSLA', 'NVDA', 'MSFT', 'GOOGL', 'AMZN', 'META', 'SPY', 'QQQ', 'AMD']
