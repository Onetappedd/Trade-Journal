...
export async function getUserTradesGroupedByDay(
  ...
  console.log(`[Calendar Matched] Fetching trades for user ${userId}:`, trades?.length || 0, 'trades found')
  ...
  console.log(`[Calendar Matched] - Open positions: ${positions.filter(p => p.openQuantity > 0).length}`)
  ...
  for (const trade of closedTrades) {
    ...
    console.log(`[Calendar Matched] ${dateKey}: ${trade.symbol} ${trade.side} - P&L: $${trade.pnl.toFixed(2)}`)
  }
