-- SnapTrade Analytics Tables Setup (Fixed)
...
DO $$ 
BEGIN
  DROP POLICY IF EXISTS "Users can view own snapshots" ON public.account_value_snapshots;
  DROP POLICY IF EXISTS "Service role can insert snapshots" ON public.account_value_snapshots;
EXCEPTION
...
DO $$ 
BEGIN
  DROP POLICY IF EXISTS "Users can view own metrics" ON public.user_performance_metrics;
EXCEPTION
  WHEN undefined_object THEN NULL;
END $$;
...
WHERE table_schema = 'public' 
  AND table_name IN ('account_value_snapshots', 'user_performance_metrics');

-- Show created functions
SELECT 
  'Analytics functions created' AS status,
  routine_name AS function_name
FROM information_schema.routines
WHERE routine_schema = 'public'
  AND routine_name IN ('take_account_snapshot', 'take_all_account_snapshots');
