...
export async function POST(request: NextRequest) {
  try {
    console.log('=== WEBULL CSV FIXED START ===');
    ...
    for (let i = 1; i < lines.length; i++) {
      try {
        ...
        if (symbol && side && quantity > 0 && price > 0 && isFilled) {
          ...
          // Use minimal required fields that should exist in the database
          const tradeData = {
            user_id: user.id,
            symbol: symbol.toUpperCase(),
            side: side === 'buy' ? 'buy' : 'sell',
            quantity,
            price: price, // Use 'price' instead of 'entry_price'
            pnl: 0,
            broker: 'webull',
    ...
    console.error('Webull CSV Fixed error:', error);
    return NextResponse.json({ 
      error: 'Import failed', 
