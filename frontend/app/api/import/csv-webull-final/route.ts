  ...
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      try {
        ...
        console.log(`Row ${i}: symbol=${symbol}, side=${side}, quantity=${quantity}, price=${price}, status=${status}, filled=${isFilled}`);
        
        if (symbol && side && quantity > 0 && price > 0 && isFilled) {
          console.log(`Valid trade found: ${symbol} ${side} ${quantity} @ ${price}`);
          
          // Use only the fields that we know exist
          const tradeData = {
            user_id: user.id,
            symbol: symbol.toUpperCase(),
            side: side === 'buy' ? 'buy' : 'sell',
            quantity,
            entry_price: price, // Use entry_price instead of price
