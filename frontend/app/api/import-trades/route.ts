...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: existingTrades } = await supabase
      .from('trades')
      .select(
        'symbol, side, quantity, entry_price, entry_date, asset_type, broker, strike_price, expiration_date, option_type',
      )
      .eq('user_id', user.id)
    ...
    for (const t of trades) {
      ...
      if (t.asset_type === 'option') {
        ...
        if (t.strike_price !== undefined && t.strike_price !== null)
          tradeData.strike_price = Number(t.strike_price);

        // Ensure all required option fields are present
        if (
          !tradeData.expiration_date ||
          !tradeData.option_type ||
          tradeData.strike_price === undefined
        ) {
          invalidTrades.push({
            trade: t,
    ...
    if (validTrades.length > 0) {
      ...
      for (let i = 0; i < validTrades.length; i += CHUNK_SIZE) {
        ...
        if (insertError) {
          ...
          console.log(
            `Successfully inserted batch ${i / CHUNK_SIZE + 1}: ${insertedData?.length} trades`,
          );
        }
      }
    }
