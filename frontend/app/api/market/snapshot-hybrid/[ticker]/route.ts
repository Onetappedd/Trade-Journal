...
export async function GET(
  ...
  const symbol = (params.ticker || '').toUpperCase().replace(/[^A-Z0-9.]/g, '').slice(0, 16);
  if (!symbol) return fail('Ticker is required', 400);
  let usingFallback = false;
  let price = 100, change = 0, changePct = 0, volume = 50000;
  try {
    if (has('FINNHUB_API_KEY')) {
      ...
      if (poly.ok) {
        ...
        if (json && json.results && Array.isArray(json.results) && json.results.length > 0) {
          ...
          price = r.c; change = r.c - r.o; changePct = price ? (change / price) * 100 : 0;
          volume = r.v;
          return ok({ data: { symbol, price, change, changePct, volume } }, false);
        }
      }
