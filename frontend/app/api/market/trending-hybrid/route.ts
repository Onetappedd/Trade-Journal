...
export async function GET(_req: Request) {
  ...
  try {
    if (has('POLYGON_API_KEY')) {
      ...
      const resp = await fetch(`https://api.polygon.io/v3/reference/tickers?market=stocks&active=true&sort=marketCap&order=desc&limit=5&apiKey=${process.env.POLYGON_API_KEY}`);
      if (resp.ok) {
