...
export async function GET(req: Request) {
  ...
  const symbols = symbolsParam.split(',').map(s => s.toUpperCase().replace(/[^A-Z0-9.]/g, '')).filter(Boolean);
    ...
    try {
      if (has('FINNHUB_API_KEY')) {
        ...
        if (resp.ok) {
          ...
          if (typeof c === 'number') {
            price = c; change = typeof d === 'number' ? d : 0; changePct = typeof dp === 'number' ? dp : (price ? (change / price) * 100 : 0);
            data.push({ symbol, price, change, changePct }); return;
          }
        }
