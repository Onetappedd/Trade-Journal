    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Debugging P&L calendar data processing
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2 font-mono text-sm">
            ...
            <div>Open Positions: {positionData?.positions.filter(p => p.openQuantity > 0).length || 0}</div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2 font-mono text-sm">
            ...
            <div>Best Day: {calendarData.bestDay.date} (${calendarData.bestDay.pnl.toFixed(2)})</div>
            <div>Worst Day: {calendarData.worstDay.date} (${calendarData.worstDay.pnl.toFixed(2)})</div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="space-y-2 text-sm">
            ...
            <ul className="list-disc list-inside space-y-1 text-muted-foreground">
              <li>Trades don't have matching buy/sell pairs → {positionData?.closedTrades.length === 0 ? '❌ No matched trades' : '✅ Has matched trades'}</li>
              <li>All trades are still open → {positionData?.positions.filter(p => p.openQuantity > 0).length === trades?.length ? '❌ All open' : '✅ Some closed'}</li>
            ...
            <div className="text-xs text-muted-foreground">
              Your trades appear to be: {trades && trades.length > 0 && trades[0].exit_price ? 'Single trades with entry/exit' : 'Separate buy/sell orders'}
            </div>
          </div>
        </CardContent>
      </Card>
