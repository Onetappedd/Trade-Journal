...
async function testPolygonAPI() {
  ...
  console.log(`   Key: ${polygonApiKey.substring(0, 8)}...${polygonApiKey.substring(polygonApiKey.length - 4)}`)
  ...
  try {
    const response = await fetch(`https://api.polygon.io/v2/snapshot/locale/us/markets/stocks/gainers?apikey=${polygonApiKey}`)
    const data = await response.json()
    
    if (data.results && data.results.length > 0) {
      console.log(`✅ Found ${data.results.length} gainers`)
      const topGainer = data.results[0]
      console.log(`   Top Gainer: ${topGainer.ticker} - $${topGainer.value} (${topGainer.change_percentage?.toFixed(2)}%)`)
    } else {
  ...
  try {
    const response = await fetch(`https://api.polygon.io/v2/snapshot/locale/us/markets/stocks/tickers/AAPL?apikey=${polygonApiKey}`)
    const data = await response.json()
    ...
    if (data.results && data.results.length > 0) {
      ...
      console.log(`   Change: ${aapl.todaysChange?.toFixed(2)} (${aapl.todaysChangePerc?.toFixed(2)}%)`)
  ...
  try {
    ...
    const response = await fetch(`https://api.polygon.io/v2/aggs/ticker/NVDA/range/1/day/${from}/${to}?adjusted=true&sort=asc&apikey=${polygonApiKey}`)
    const data = await response.json()
  ...
  try {
    const response = await fetch(`https://api.polygon.io/v3/reference/tickers?search=Tesla&active=true&limit=5&apikey=${polygonApiKey}`)
    const data = await response.json()
