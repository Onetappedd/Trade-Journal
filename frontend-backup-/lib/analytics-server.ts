...
const API_BASE_URL =
  (process.env.PYTHON_API_BASE_URL as string) ||
...
function mapAssetTypeToClasses(
  assetType?: string,
): ('stocks' | 'options' | 'futures')[] | undefined {
...
export async function fetchEquityCurve(
  req: Omit<EquityCurveRequest, 'userTimezone'> & { userTimezone?: string },
) {
...
export async function fetchMonthlyPnl(
  req: Omit<MonthlyPnlRequest, 'userTimezone'> & { userTimezone?: string },
) {
  const token = await getAuthToken();
...
export async function fetchCardsSummary(
  req: Omit<CardsRequest, 'userTimezone'> & { userTimezone?: string },
) {
  const token = await getAuthToken();
